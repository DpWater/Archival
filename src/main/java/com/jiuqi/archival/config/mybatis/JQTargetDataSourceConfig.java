 ...
 */
@Configuration
@MapperScan(basePackages = { "com.jiuqi.archival.mapper.gdtarget" }, sqlSessionFactoryRef = "jqTargetSqlSessionFactory")
public class JQTargetDataSourceConfig extends DataSourceBaseConfig {
    /** XML配置文件存放路径 */
    private final String MAPPER_LOCATION = "classpath:/mapper/gdtarget/*.xml";

    /** 别名包 */
    private final String TYPE_ALIASES_PACKAGE = "com.jq.target.bean";

    @Value("${jqtargetdb.datasource.url}")
    private String url;

    @Value("${jqtargetdb.datasource.username}")
    private String username;

    @Value("${jqtargetdb.datasource.password}")
    private String password;

    @Value("${jqtargetdb.datasource.driverClassName}")
...
//	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
     ...
     * 配置事务
     */
    @Bean(value = "transactionManager_jqtarget")
//	@Primary//该Bean会被优先考虑。在多数据源的环境中必须且只加在其中一个配置里
     ...
     * @throws Exception
     */
    @Bean(value = "jqTargetSqlSessionFactory")
    public SqlSessionFactory getSqlSessionFactory(@Qualifier("jdTargetSource") DataSource dataSource) throws Exception {
